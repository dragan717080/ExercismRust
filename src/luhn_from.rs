...
impl Luhn {
    pub fn is_valid(&self) -> bool {
        ...
        // Replace every second digit starting from right
        let replaced = self.digits.iter().filter_map(|c| c.to_digit(10)).rev().enumerate().map(|(index, digit)| if index % 2 == 0 { digit } else { digit * 2 }).map(|digit| if digit > 9 { digit - 9 } else { digit });
